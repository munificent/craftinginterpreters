...
Pratt's technique is for hand-written ones, so it's mostly overlooked.

But in production compilers, where hand-rolled parsers are actually quite
common, you'd be surprised how many people know it. Ask where they learned it,
and it's always, "Oh, I worked on this compiler years ago and my coworker said
they took it from this old front end..."
...
parse a single expression. We aren't going to do statements yet, so that's the
only subset of the grammar we support. We'll revisit this when we [add
statements in a few chapters][globals]. Once we compile the entire expression,
...
function definition and see what it calls, then *those* functions and so on,
...
it's the best I could do. I hope you find it's worth the effort.
...
languages. The compiler only has a peephole view into the user's program while
parsing source and generating code. The language must be designed such that you
can figure out what code to generate without needing much surrounding context
and without needing to look ahead at later parts of the program. <span
name="lox">Fortunately</span>, tiny, dynamically-typed Lox is well-suited to
that.
...
Just like in jlox, it advances to the next token in the source code. It asks the
...
leaves it up to the parser to detect those and report the error. We do that
here.

We keep looping, reading tokens and reporting the errors, until we hit a
non-error one or reach the end. That way, the rest of the parser doesn't have to
deal with error tokens floating around confusing things.
...
If the scanner hands us an error token, we need to actually tell the user. This
function reports the given error message:

^code error-at-current

It pulls the location out of the current token in order to tell the user where
the error occurred. More often, we'll report an error at the location of the
token we just consumed, so we give the shorter name to this other function:
...
format string. Using `*` instead of a number lets you pass the precision as an
argument. So that `fprintf()` call prints the first `token->length` characters
of the string at `token->start`. We need to limit the length like that because
the lexeme points into the original source string and doesn't have a terminator
at the end.
...
emit a series of bytecode instructions that encode those semantics. It doesn't
get more fundamental than writing a single byte to the chunk:
...
after reading the token whose source we're generating bytecode for.
...
change a lot of code, I encapsulated that logic in this function.
...
around a table of function pointers and data, I'm not ready to dump the whole
table on you yet.

We'll work our way to that by first showing the functions it contains.
Eventually, it will all come together. Feel free to skip ahead and bounce around
the chapter if it helps.
...
at that index is the code to compile an expression of that token type.
...
instruction to load that constant and push it onto the stack. To insert an entry
in the constant table, we rely on:
...
Our array of parsing function pointers works OK. It's easy to add new syntax to
the grammar by plugging in a new function at the right index in the array. It's
also easy to see which tokens correspond to expressions by seeing which array
slots are in use.

Alas, few expressions are only a single token. However, many expressions *start*
with a particular token. We call these *prefix* expressions. For example, if we
expect to parse an expression, and the current token is `(`, we know we must be
looking at a parenthesized grouping expression.
...
It's unnecessary right now, but this will make more sense when we add
the `!` operator in [the next chapter][next].
...
little weird to emit the negate instruction *after* its operand since the `-`
...
all of the remaining code including the `+`.
...
lowest allowed precedence, we'd get the others too.
...
We use the unary's own `PREC_UNARY` precedence to permit <span
name="useful">nested</span> unary expressions like `!!doubleNegative`. Since
unary operators have pretty high precedence, that correctly limits the operand
to a call expression or higher. When we call `unary()` to parse:

<aside name="useful">

Not that nesting unary expressions is particularly useful in Lox. But other
languages let you do it, so we do to.
...
kind of expression we are parsing from its very first token. With infix
expressions, we don't even know we're in the middle of a binary operator until
*after* we've parsed its left operand and then stumbled onto the operator token
in the middle.
...
that we're in the middle of an infix expression and that the prefix expression
we already compiled is actually an operand to that.
...
parser functions with token types. In the row for `TOKEN_PLUS`, we point to
this function:
...
also know we need some a table that, given a token type, lets us find:
...
tokens it needs, and returns back here. That's really all there is to it, at
least for prefix expressions.

Infix expressions are where it gets challenging, since precedence comes into
play. The implementation is remarkably simple:

^code infix (1 before, 1 after)

That's the whole thing. Really. Here's how it works: When you call it, it looks
up a prefix parser for the current token. The first token is *always* going to
belong to some kind of prefix expression, by definition. It may turn out to be
nested as an operand inside one or more infix expressions, but as you read the
code from the left to right, the first thing you hit is always some prefix
expression.
...
only if the precedence level that the call to `parsePrecedence()` is restricted
to includes that infix operator.

If the next token is too low precedence, or isn't an infix operator at all,
we're done. We've parsed as much expression as we can. Otherwise, we hand off
control to the infix parser we found. It consumes whatever other tokens it needs
(usually the right operand) and returns back to `parsePrecedence()`. Then we
loop back around and see if the *next* token is also a valid infix operator that
can take the entire preceding expression as its operand.

We keep looping like that, crunching through infix operators and their operands
until we hit a token that isn't an infix operator or is too low precedence.
That's fairly straightforward, but it's mentally slippery since each of those
prefix and infix parsers often calls back into `parsePrecedence()` for its
operands.

That's a lot of prose, but if you really want to mind meld with Vaughan Pratt
and fully understand the algorithm, it's worth stepping through the parser in
...
broken code. That's OK, because it won't get executed, but we'll just confuse
ourselves if we try to read it.

Finally, to access `disassembleChunk()`, we need to include its module:
