...
lighthearted frolic through a daisy meadow. But while the task is easy -- my
favorite kind -- the reward is surprisingly large.

[statements]: statements-and-state.html

Right now, our interpreter is little more than a calculator. A program only does
a fixed amount of work before completing. To make a program run twice as long
you have to make the source code twice as lengthy. In this chapter, our
interpreter takes a big step toward the programming language major leagues:
*Turing-completeness*.
...
stability of the foundation that all math was built upon. To address that
[crisis][], they decided to start over. Starting from a handful of axioms,
...
it was valid. Naturally, given mathematician's predilection for self reference,
sets can contain all sorts of things, even other sets. So Russell, rascal that
he was, came up with:
...
fact, the two questions are deeply intertwined. This is a really fascinating
corner of both programming and mathematics that touches fundamental questions
about what brains are able to do and how the universe works. I can't do it
justice here.
...
self-aggrandizing to put his own name on them. Later mathematicians did that for
him. That's how you get famous while still retaining some modesty.
...
Alonzo Church yet -- but the two formalism are [equivalent in power][thesis]. In
fact, any programming language with some minimal level of expressive is powerful
enough compute *any* computable function.
...
*   **Looping** control flow executes a chunk of code more than once.
    It's jumping *back* so that you can do something again. Since you don't
    usually want *infinite* loops, it typically has some conditional logic to
    know when to stop looping as well.
...
For simplicity's sake, Lox doesn't have a conditional operator, so we'll start
with the if statement. It gets a new production under the statement grammar
rule:
...
can't tell when it's reached the end of the condition expression. But the
opening parenthesis after `if` doesn't do anything useful. Dennis Ritchie put it
there so he could use `)` as the ending delimiter without having unbalanced
parentheses.
...
clause by looking for the preceding `else` keyword. If there isn't one, the else
...
end of the if statement, the grammar ambiguous when you nest ifs in this way.
...
Instead, most languages and parsers avoid the problem in an ad hoc way. Whatever
hack they use to get themselves out of the trouble, they always choose the same
interpretation -- the else is bound to the nearest if that precedes it.
...
The code is barely more than a thin wrapper around the self-same Java code. It
...
implemented, the part that makes control flow special is that Java `if`. Most
other syntax trees always evaluate their subtress. Here, we may not evaluate the
then or else statement. If either of those has a side effect, the choice not to
evaluate it becomes user-visible.
...
isn't going to be the case, so there's no need to call `sideEffect()` and it
gets skipped.

This is why we didn't implement the logical operators with the other binary
operators. Now we're ready. The logical operators are low in the precedence
table, just above assignment. As in C, they each have their <span
name="logical">own</span> precedence with `||` lower than `&&`. We slot them
...
look at its value and operator to see if we can short circuit and return.
Otherwise, and only then, do we evaluate the right operand.
...
for statement by implicitly mapping to a iteration protocol that the object
...
defining an iteration protocol that the for loop could call. So we'll stick with
...
already do with the statements we already have. If for loops didn't support
...
Oh, how I wish the accepted term for this were "caramelization". Why introduce a
...
in. Desugaring bridges that gap -- it lets you take a rich expressive syntax and
cook it down to a small number of primitives that the back end can do its magic
on.
...
Here is where it's going to get interesting. The desugaring is going to happen
here, so we'll build this method a piece at a time, starting with the opening
...
grammar that allows either an expression or a statement. That wasn't *entirely*
true, I guess.
...
operation maps a single syntactic unit, or some decadent dessert where every bit
of behavior can be expressed ten different ways? Successful languages
inhabit all points along this continuum.

On the extreme acrid end are languages with ruthlessly minimal syntax like Lisp,
...
sugar. Instead, minimal syntax it does provide if flexible enough to let library
code be as beautiful as if it were baked into the grammar.
...
semantics. Once added, you can never take it away, so languages tend to
