...
1.  To interpret `OP_NEGATE`, we pop the operand, negate the value, and then
    ...
    decrements `ip` unnecessarily, since the stack ends up the same height in
    the end. It might be faster to simply negate the value in place on the stack
    and leave `ip` alone. Try that and see if you can measure a performance
    difference.
