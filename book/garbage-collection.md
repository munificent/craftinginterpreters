...
We say Lox is a "high-level" language because it frees programmers from having
to worry about details irrelevant to the problem they're trying to solve. The
user becomes an executive giving the machine abstract goals and letting the
lowly computer figure how to get there.
...
automatically allocates. They never worry about deallocating anything. As long
as they are using a piece of memory, the machine ensures that memory is still
there.
...
languages are older than Earth Day, so the inventors went with the simile they
knew.
...
having a time machine, how can an implementation tell what code the program will
execute and which data it *will* use? Spoiler alert: VMs cannot travel into the
future. Instead, the language makes a <span
...
read? Actually, no. Here's an example:
...
reference to that string. It is unreachable and we can safely free it. A value
is **reachable** if there is some way for a user program to reference it.
...
the VM stores references to objects that it can find.

Other values aren't directly accessible, but can still be found by going through
a reference inside some other value. Fields on instances of classes are the most
obvious case, but we don't have those <span name="class">yet</span>. Even
without those, our VM still has indirect references. Consider:
...
1.  At a suitable time, run a garbage collection.
...
narrow topic, it is quite enjoyable to read.
...
invented, right after Fortran. John McCarthy considered manual memory management
and reference counting, but <span name="procrastination">eventually</span>
settled on (and coined) garbage collection -- once the program was out of memory
it would go back and find unused storage it could reclaim.
...
examples were being done." Our choice to procrastinate adding the GC to clox has
a luminous legacy.
...
*   **Sweeping.** Once the mark phase completes, every reachable object in the
    ...
    reclamation. We go through them all and free each one.
...
That's what we're gonna implement. Whenever we decide it's time to reap some
bytes, we'll trace everything and mark all the reachable objects, free what's
...
Whenever we call `reallocate()` to acquire more memory, we force a full
...
Otherwise, the VM can get into a state where it needs more memory but never
collects any.

<aside name="demand">

More sophisticated collectors might run on a separate thread, or interleaved
periodically during program execution, a function call boundaries, or when a
...
and require no heap allocation. The garbage collector doesn't need to worry them
at all, so the first thing we do is ensure that the value is an actual heap
object. If so, the real work happens in this function:
...
this function directly from other code and in some of those cases, the object
...
While we're in here, let's log when an object gets marked:
...
accessible to the user's program. But the VM has a few of its own hidey holes
where it squirrels away references to values that it directly accesses.

Most data lives in the value stack, but the VM maintains a separate stack of
...
compiler -- can get to *without* going through some other object has its mark
bit set.
...
the constants contained in the function's body. This is enough to build a fairly
...
*   **Black** <img src="image/garbage-collection/black.png" class="dot" />
    &ndash; When we take a gray object and traverse and mark all of the objects
    it references, we then turn it black. This color means the mark phase is
...
3.  As long as there are still gray objects:
    ...
    1.  Pick a gray object. Look at each other object it references and mark it
        gray if the object is white.
...
of reachable black objects behind it. Unreachable objects are not touched by the
...
space-time continuum. So we'll manage its memory ourselves, explicitly.
...
of pointers to the upvalues it captured.
...
it is an unmarked white object, we unlink it from the list and free it using the
`freeObject()` function we already wrote.
...
pointer to every single string in the heap. The VM uses this to de-duplicate
strings.

During the mark phase, we deliberately did *not* treat the VM's string table as
a set of roots. If we had, no <span name="intern">string</span> would *ever* be
collected. The string table would grow and grow and never yield a single byte of
memory back to the operating system. That would be bad.

<aside name="intern">

This can be a real problem. Java does not intern all strings, but it does intern
string literals and also provides an API to let users explicitly add strings to
the string table. For many years, the capacity of that table was fixed and
strings added to it could never be removed. If users weren't careful about their
use of `String.intern()` they could run out of memory and crash.

Ruby had a similar problem for years where symbols were not garbage collected.
Both eventually enabled the GC to collect these strings.
...
string, but that link should not be considered an edge when determining
reachability. That implies that the referenced object can be freed. When that
happens, the dangling pointer must be fixed too, sort of like a magic
self-clearing pointer. This particular set of semantics for a reference comes up
frequently enough that it has a name: a [**weak reference.**][weak]
...
But, when the stress testing flag is off, it never runs at all. It's time  to
decide when the collector should be invoked during normal program execution.
...
Evvery managed language has some performance cost compared to requiring the user
to explicitly free memory. The time spent actually freeing memory is the same,
but the GC adds runtime overhead spent figuring out *which* memory to free. That
is time *not* spent running the user's code and doing useful work. In our
implementation, that's the entirety of the mark phase. The goal of a
sophisticated garbage collector is to minimize that overhead.
...
The bar represents the execution of a user program, divided into time spent
running user code and time spent in the GC. The size of the largest single slice
of time running the GC is the latency. The size of all of the user code slices
added up is the throughput.
...
can serve to customers in a single day. Latency is the how long the unluckiest
...
collector**. In other words, hire some dedicated dishwashers to wash while
others keep baking. This is how very sophisticated GCs work because it does let
the bakers -- the worker threads -- keep running user code with little
interruption.
...
the user's program doesn't have enough time to even generate new garbage for it
to collect. The VM will spend all of its time obsessively revisiting the same
...
collector runs is one of the main knobs we have to tune the trade-off between
latency and throughput.
...
size of the heap. We'll keep track of the total number of bytes of managed
memory that the VM has allocated so far. When it goes above a threshold, we
trigger a GC. After that, we note how many bytes of memory remain -- how many
were *not* freed. Then we adjust the threshold to some value larger than that.
...
long. If we had some real-world Lox programs, we could profile those to tune
this. But since all we have is toy programs, I just picked a number.
...
real-world programs it is actually intended for. A GC tuned using
micro-benchmarks rarely performs well out in nature.

</aside>

Every time we allocate or free some memory, we adjust the counter by the delta:
...
Our jobs are to ensure the collector finds and *free* any object that isn't used
while *preventing* it from freeing any object that can be used. In both cases,
mistakes are easy to make and not quickly obvious. Slow memory leaks and memory
accesses that still *look* right even though the memory is technically no longer
ours can let the VM keep running well past the point that the erroneous code has
executed.
...
in some local variable on the C stack. The GC walk's the *VM's* value and
...
Many precise GC walk the C stack too. Even those have to be careful about
...
back off. Most times, I said this was for the GC's benefit. Now you see why. The
work it did between pushing and popping potentially allocated memory and thus
could trigger a GC. We had to make sure the object was on the value stack so
that the collector's mark phase would find it and keep it alive.
...
few. Give it a try and then I'll walk you through them.
...
A collector burns throughput if it spends a long time re-visit objects that are
still alive. But it can increase latency if it avoids collecting and accumulates
a large pile of garbage to wade through. If only there were some way to tell
which objects were likely to be long-lived and which weren't. Then the GC could
avoid revisiting the long-lived ones as often and clean up the ephemeral ones
more often.

Spoiler alert, there kind of is. Many years ago, GC researchers gathered some
metrics on the lifetime of objects in real-world running programs. They tracked
every object when it was allocated and eventually when it was no longer needed
and then graphed out how long objects tended to live.
...
die young, the garbage collector is invoked pretty <span
