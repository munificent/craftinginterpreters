...
statement, we used a Java if statement to select which branch to execute. That
works, but isn't entirely satisfying. How does *Java* implement if statements?

For clox, we'll take a different approach. Partially this is to dispell a little
more magic around how languages do control flow. But mostly it's because we have
to due to the nature of our VM. Compilation to bytecode strongly encourages us
to implement control flow at a lower level than we have before.

First, let's take a step back. When we talk about "control flow", what are we
referring to? By "flow" we mean the way execution moves through the text of the
program. Almost like there is a little robot inside the computer wandering
through our program, executing each piece of code. Flow is the path that robot
takes, and by *controlling* it, we drive which pieces of code it executes.
...
value of that field is exactly where we in the program right this moment.
...
The VM evaluates the condition -- executes the generated bytecode for the
condition expression. If the result is truthy, then it executes the "then"
branch -- here a print statement. The interesting case is when the condition is
falsey. When that happens, execution skips over the then branch and proceeds to
the next statement.

To skip over a chunk of code, we can simply set the `ip` field to the address
of the next bytecode instruction after that code. To *conditionally* skip over
some code, we need an instruction that looks at the value on top of the stack.
If it's falsey, it adds a given offset to the `ip` to jump over a range of
...
But *we* certainly do. If we emit bytecode that, say, jumps out of a scope into
another one, then the stack could be in an unknown, inconsistent state. So even
though our VM is unstructured, we'll take care to ensure that our compiler only
generates clean code that maintains the same structure and nesting that Lox
itself does.

This is exactly how real CPUs behave. Even though we might program them using
higher level languages that only support structured control flow, the compiler
...
The `)` is useful because it separates the condition expression from the body.
Some languages use a `then` keyword instead. But the `(` doesn't do anything.
It's just there because an unmatched `)` looks wrong to us humans.
...
The first emits a bytecode instruction and write a placeholder operand for the
jump offset. We pass in the op code itself because we'll eventually have a
couple of kinds of jump instructions that are emitted using this helper. We use
two bytes for the jump offset operand. A 16-bit <span
name="offset">offset</span> lets us jump over up to 65,536 bytes of code, which
...
It returns the byte location of the emitted instruction in the chunk's bytecode.
After compiling the then branch, we take that location and pass it to:

^code patch-jump

This goes back into the bytecode and replaces the operand at the given location
with the calculated jump offset. To calcualte the offset, it assumes you want to
jump from the location of that instruction to just after the most recently
bytecode. Thus, it's important for us to call this helper right before we emit
the next instruction that should execute after the jump is taken.

In the case of an if statement, that means right after we compile the then
...
it will pick up execution at that new instruction, after the end of all of the
...
we aren't totally done here, since this technically leaves an extra value
floating around on the stack. We'll clean that up soon. Ignoring that for the
moment, we do have a working if statement in Lox now, with only one little
instruction required to support it at runtime in the VM.

### Else clauses

We're about halfway to supporting a full if statement. The last half is the else
clause. After we compile the then branch, we look for an `else` keyword. If we
find one, we compile the else branch:
...
This way, after executing the then branch, we jump to the next statement after
the else. Unlike the other jump, this jump is unconditional. We always take it,
so we need another instruction that expresses that:
...
clean up those condition values we left on the stack. Remember, each statement
is required to have zero stack effect -- after the statement is finished
executing, the stack should look the same as it did before.

We could have the `OP_JUMP_IF_FALSE` instruction pop the condition itself, but soon we'll use that same instruction for the logical operators where we don't want the condition popped. Instead, we'll have the compiler emit a couple of explicit `OP_POP` instructions when compiling an if statement. We need to take care that each execution path through the generated code pops the condition.
...
would take care of itself. Alas, the <span name="80s">80s</span> are over, so
we'll have to grind it out ourselves.
...
short-circuit and don't evaluate their right operand sometimes depending on the
value of the left operand, they work more like control flow expressions.
...
compiled. That means at runtime, its value will be on the top of the stack. If
that value is falsey, then we know the entire `&&` must be false, so we skip the
...
value needs to stick around to be the result of the entire expression.
...
VM, it's worth exploring adding more specialized instructions and see how that
affects performance.
...
implement it without adding any new instructions. Forgive me my indulgences.

OK, those are the three *branching* constructs in Lox. By that, I mean, these
are the control flow features that only jump *forward* over code -- they only
*skip* some code. Other languages often have some kind of multi-way branching
statement like `switch` and maybe a conditional expression like `?:`, but Lox
keeps it simple.
